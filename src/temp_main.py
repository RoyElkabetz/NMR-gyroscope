...
wd = g129 * 1 * phy.G2T * np.ones_like(ts)                     # rad / s


# initialize Xenon
Xe129 = xe.Xenon(gamma=g129, t1=t1, t2=t2, name='129')
...
env129.set_state(wr=wr[0], B0=B0[0], Bnoise=Bnoise[0], Ad_y=Ad[0], wd_y=wd[0])
...
for i, t in enumerate(ts):
    env129.set_state(wr=wr[i], B0=B0[i], Bnoise=Bnoise[i], Ad_y=Ad[i], wd_y=wd[i])
...
utils.plot_results(ts, Kt, Ks, Xe129, env129, wr)