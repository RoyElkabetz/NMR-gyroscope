...
class Xenon:
    ...
    def plot_results(self, environment, ti=0):
        ...
        ax1.set_ylabel("$K$")
        ax1.plot(ts[ts > ti], self.Kt[[ts > ti], 0], label='$K_x$', color='tab:blue')
        ax1.plot(ts[ts > ti], self.Kt[[ts > ti], 1], label='$K_y$', color='tab:orange')
        ax1.plot(ts[ts > ti], self.Kt[[ts > ti], 2], label='$K_z$', color='tab:green')
        ax1.plot(ts[ts > ti], self.Ks[[ts > ti], 0], '--', label='$K_x$ - steady state solution', color='tab:blue')
        ax1.plot(ts[ts > ti], self.Ks[[ts > ti], 1], '--', label='$K_y$ - steady state solution', color='tab:orange')
        ax1.plot(ts[ts > ti], self.Ks[[ts > ti], 2], '--', label='$K_z$ - steady state solution', color='tab:green')
        ax1.legend()
        ax1.grid(True)

        ax2 = plt.subplot(3, 2, 3)
        ax2.plot(ts[ts > ti], self.Kt_perp[ts > ti], label='$|K_{\perp}|$', color='tab:blue')
        ax2.plot(ts[ts > ti], self.Ks_perp[ts > ti], '--', label='$|K_{\perp}|$ steady state solution', color='tab:blue')
        ...
        ax3.plot(ts[ts > ti], self.phase_perp[ts > ti] * phy.R2D, label='$\phi = arctan(K_y/K_x)$')
